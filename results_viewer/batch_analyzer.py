...
class BatchAnalyser(unittest.TestCase):
    ...
    def test_compute_metrics_for_csv(self):
        ...
        csv_file="Allo8_S010W005.hist.csv" #"Allo6_S030W010.hist.csv"
        max_Ks=1.0
        output_folder = "/home/tamsen/Data/Specks_outout_from_mesx/sim35_log/analysis"
...
def analyze_histogram(bins, n, WGD_time_MYA, SPC_time_MYA,
    ...
    minima = find_local_minima(bins, smoothed_ys)
    maxima = find_global_maxima(bins, smoothed_ys)
    plt.scatter([m[0] for m in minima], [m[1] for m in minima], color="red", label="minima", marker='*')

    ssd_end, next_min =smallest_min(minima,maxima)
