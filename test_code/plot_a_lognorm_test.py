...
class LognormTestCase(unittest.TestCase):
    ...
    def test_a_lognorm(self):
        ...
        max_Ks = 1.0
        bin_size = 0.01
        xs = np.arange(0, max_Ks + 0.01, bin_size)
        fig, ax = plt.subplots(1, 1, figsize=(10, 10))

        title='lognorm'
        fig.suptitle(title)
        #ys=[norm.pdf(x,mean,sd) for x in xs]
        shape_parameter=0.8
        xscale = 0.2
        #ys = [lognorm.pdf(xscale*x,shape_parameter) for x in xs]
